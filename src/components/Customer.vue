<template>
  <div v-if="this.customer">
    <h4>Customer</h4>
    <div>
      <label>Name: </label> {{this.medicamento.name}}
    </div>
    <div>
      <label>Age: </label> {{this.medicamento.age}}
    </div>
    <div>
      <label>Active: </label> {{this.medicamento.active}}
    </div>
...
export default {
  ...
  methods: {
    ...
    updateActive(status) {
      ...
      http
        .put("/medicamento/" + this.customer._id, data)
        .then(response => {
    ...
    deleteCustomer() {
      http
        .delete("/medicamento/" + this.customer._id)
        .then(response => {
          console.log(response.data);
