  ...
  <div class="col-md-12">
    <div class="mt-2 center-block">
      ...
      <div v-for="(pedido, i) in pedidos.data" :key="i">
        <h5 style="padding-top: 20px; color: #6991E1">Estado: {{pedido.state}}, N° del pedido:
          {{pedido.numberPedido}}</h5>
        ...
        <button v-if="pedido.state != 'Aprobado'" :disabled="!isAuth ? '' : disabled" class="btn btn-success btn-lg"
                style="text-align: center" v-on:click="actualizarPedido(pedido)" role="button">Aprobar
        </button>
      </div>
    </div>
  </div>
